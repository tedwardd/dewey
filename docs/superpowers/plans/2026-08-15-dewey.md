...
**Goal:** Build `dewey`, a Rust CLI framework that accesses open ebook libraries through standalone modules speaking JSON-RPC over stdio, with working Project Gutenberg and Standard Ebooks modules.
...
**Spec:** `docs/superpowers/specs/2026-08-15-dewey-design.md` (approved).
...
- **Config:** `~/.config/dewey/config.toml`, keys `default_module`, `download_dir` (tilde-expanded). Missing file OK; invalid file → exit 4.
- **Discovery:** `DEWEY_MODULES` env var overrides the default modules dir `~/.config/dewey/modules`.
...
- **Modules:** Python 3 stdlib only. When `DEWEY_FIXTURE=<dir>` is set, modules MUST answer from fixture files and never touch the network. Fixture filenames: `<method>.json` (no params) or `<method>-<slug>.json` where slug = lowercase, non-alphanumerics → `-` (URLs keyed by last path segment).
...
**Interfaces:**
- Consumes: `CliError`.
- Produces: `Config { default_module: Option<String>, download_dir: Option<String> }` (Deserialize, Default), `Config::expanded_download_dir() -> Option<String>`; `config_dir() -> Result<PathBuf, CliError>` (`$HOME/.config/dewey`, `HOME` unset → Config error); `modules_dir() -> Result<PathBuf, CliError>` (`DEWEY_MODULES` env override, empty value → Config error; else `config_dir()/modules`); `load_config() -> Result<Config, CliError>` (missing file → default; parse/validation errors → Config); `load_config_from(dir: &Path) -> Result<Config, CliError>` (testable variant reading `dir/config.toml`); `expand_tilde(s: &str) -> String`.
...
fn bin() -> Command {
    Command::new(env!("CARGO_BIN_EXE_dewey"))
}
...
Run: `printf '%s\n' '{"jsonrpc":"2.0","id":1,"method":"search","params":{"query":"moby dick","limit":5}}' | DEWEY_FIXTURE=modules/gutenberg/fixtures python3 modules/gutenberg/module.py`
...
Run: `printf '%s\n' '{"jsonrpc":"2.0","id":1,"method":"categories","params":{}}' | DEWEY_FIXTURE=modules/standard-ebooks/fixtures python3 modules/standard-ebooks/module.py`
...
Modules live in `~/.config/dewey/modules/<name>/` (override with the
`DEWEY_MODULES` env var). Install from a directory:
...
```bash
dewey modules                       # list installed modules
dewey search "moby dick" --module gutenberg
dewey categories --module standard-ebooks
dewey list --category <feed-url> --module standard-ebooks
dewey show 2701 --module gutenberg
dewey download 2701 --format epub --module gutenberg
dewey download <book-id> --format epub --dir ~/Downloads
```

Default module and download dir go in `~/.config/dewey/config.toml`:
...
- Fixture mode: set `DEWEY_FIXTURE=<dir>` to serve recorded responses
